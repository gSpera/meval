...
import (
	"fmt"
	"github.com/gSpera/meval/internal/scanner"
	"strconv"
	"strings"
	"unicode"
...
func assert(cond bool, msg string) {
	if !cond {
		panic("github.com/gSpera/meval/internal/parser internal error: " + msg)
	}
}
...
func isValidImport(lit string) bool {
	const illegalChars = `!"#$%&'()*,:;<=>?[\]^{|}` + "`\uFFFD"
	s, _ := strconv.Unquote(lit) // github.com/gSpera/meval/internal/scanner returns a legal string literal
	for _, r := range s {
